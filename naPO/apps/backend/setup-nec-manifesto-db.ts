...
async function setupNecManifestoDB() {
  console.log('\n=== Setting up NEC Manifesto API in Database ===\n');
  ...
  try {
    ...
    if (existing) {
      ...
      console.log('Creating new API source: public_data_election');

      // Create API source
      const apiSource = await prisma.apiSource.create({
        data: {
          name: 'public_data_election',
...
async function createApiKey(sourceId: string) {
  const apiKey = process.env.NEC_MANIFESTO_API_KEY;

  if (!apiKey) {
    console.error('✗ NEC_MANIFESTO_API_KEY not found in .env file');
    throw new Error('NEC_MANIFESTO_API_KEY is required');
  }
...
setupNecManifestoDB().catch((error) => {
