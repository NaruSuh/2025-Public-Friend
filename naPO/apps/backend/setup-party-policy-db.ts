...
async function setupPartyPolicyDB() {
  console.log('\n=== Setting up Party Policy API in Database ===\n');

  try {
    // Check if API source already exists
    const existing = await prisma.apiSource.findFirst({
      where: { name: 'public_data_party_policy' },
    });
    ...
    if (existing) {
      ...
      console.log('Creating new API source: public_data_party_policy');

      // Create API source
      const apiSource = await prisma.apiSource.create({
        data: {
          name: 'public_data_party_policy',
...
setupPartyPolicyDB().catch((error) => {
